            ...
            <div className="fixed inset-0 z-50 bg-white dark:bg-gray-900 p-8 md:p-12 flex flex-col animate-fade-in">
                <div className="flex justify-between items-center mb-8 border-b pb-4 dark:border-white/10">
                    ...
                    <button
                        ...
                        className="p-3 bg-gray-100 dark:bg-white/10 rounded-full hover:bg-gray-200 transition-colors"
                    >
                    ...
                    {notices.length === 0 ? (
                        ...
                        notices.map((notice, idx) => (
                            <div key={notice.id} className="flex gap-6 items-start">
                                ...
                                <p className="text-3xl md:text-4xl text-gray-800 dark:text-gray-100 font-medium leading-relaxed">
                                    {notice.text}
                                </p>
                            </div>
                        ))
        ...
        <div className="bg-white dark:bg-surface-dark rounded-2xl p-6 shadow-sm border border-gray-100 dark:border-white/5 h-full flex flex-col">
            ...
            <form onSubmit={handleAdd} className="flex gap-2 mb-4">
                <input
                    ...
                    className="flex-1 bg-gray-50 dark:bg-white/10 dark:text-white border-none rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-primary/20 outline-none"
                />
                <button
                    type="submit"
                ...
                {notices.length === 0 ? (
                    ...
                    notices.map(item => (
                        <div key={item.id} className="group flex items-center justify-between p-2 rounded-lg hover:bg-gray-50 dark:hover:bg-white/5 transition-colors text-sm">
                            <span className="text-gray-700 dark:text-gray-300 line-clamp-2">{item.text}</span>
                            <button
